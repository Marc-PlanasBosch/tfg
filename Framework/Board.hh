 ...
 * Ara utilitzem l'enum del UnitTypeRegistry per compatibilitat
 */
// L'enum UnitType està definit a UnitTypeRegistry.hh
...
class Board {
    ...
    // Unitats dinàmiques
    map<string, int> nb_units_by_type_;  // Nombre d'unitats per tipus
    map<string, int> unit_health_by_type_;  // Salut per tipus
    map<string, int> unit_regen_by_type_;   // Regeneració per tipus
    map<string, int> unit_special_params_;  // Paràmetres especials per tipus
    ...
    //Aux vectors dinàmics
    map<string, vector< vector<int> > > units_by_player_;  // Unitats per jugador per tipus
    bool vectors_by_player_updated_;
    
    /**
     * Construct a board by reading first round from a stream.
     */
    Board (istream& is, bool secgame, int seed = -1);
...
public:
     ...
     */
    static GameConfiguration game_config;
    ...
    /**
     * Returns the number of units of a specific type each player starts with
     */
    inline int nb_units_of_type(const string& type) const {
        auto it = nb_units_by_type_.find(type);
        return (it != nb_units_by_type_.end()) ? it->second : 0;
    }

    /**
     * Returns the initial (and maximum) health of a specific unit type
     */
    inline int unit_health(const string& type) const {
        auto it = unit_health_by_type_.find(type);
        return (it != unit_health_by_type_.end()) ? it->second : 100;
    }
    
    /**
     * Returns the health a unit type recovers per round when not moving
     */
    inline int unit_regen(const string& type) const {
        auto it = unit_regen_by_type_.find(type);
        return (it != unit_regen_by_type_.end()) ? it->second : 0;
    }
    
    /**
     * Returns a special parameter for a unit type
     */
    inline int unit_special_param(const string& type, const string& param) const {
        string key = type + "_" + param;
        auto it = unit_special_params_.find(key);
        return (it != unit_special_params_.end()) ? it->second : 0;
    }
    
    // Mètodes de compatibilitat per Battle Royale
    inline int nb_farmers () const { return nb_units_of_type("farmers"); }
    inline int nb_knights () const { return nb_units_of_type("knights"); }
    inline int farmers_health () const { return unit_health("farmers"); }
    inline int knights_health () const { return unit_health("knights"); }
    inline int farmers_regen () const { return unit_regen("farmers"); }
    inline int knights_regen () const { return unit_regen("knights"); }
    ...
    /**
     * Returns all units of a specific type for a player
     */
    inline const vector<int>& units_of_type(int player, const string& type) {
        if (!vectors_by_player_updated_) update_vectors_by_player();
        return units_by_player_[type][player];    
    }
    
    // Mètodes de compatibilitat per Battle Royale
    inline const vector<int>& farmers(int player) {
        return units_of_type(player, "farmers");
    }
    
    inline const vector<int>& knights(int player) {
        return units_of_type(player, "knights");
    }
    ...
    /**
     * Mètodes per gestionar configuració dinàmica
     */
    void loadDynamicConfiguration();
    void generateUnitsDynamically();
